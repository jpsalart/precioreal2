export default async function handler(req, res) {
  ...
  } catch (e) { out.ping = { mode: "header", ok: false, error: String(e) }; }
  ...
  } catch (e) { out.pingQuery = { mode: "query", ok: false, error: String(e) }; }
  ...
  } catch (e) { out.write = { mode: "header", ok: false, error: String(e) }; }

  // SET/GET (query _token)
  const k2 = `kv:diag:q:${Date.now()}`;
  try {
    const r1 = await fetch(`${url}/set/${encodeURIComponent(k2)}/${Date.now()}?EX=60&_token=${encodeURIComponent(token)}`, { method: "POST" });
    const r2 = await fetch(`${url}/get/${encodeURIComponent(k2)}?_token=${encodeURIComponent(token)}`);
  ...
  } catch (e) { out.writeQuery = { mode: "query", ok: false, error: String(e) }; }
